...
pub mod chunk_pos {
    ...
    pub const MARKER: u64 = packed(&Vec2::new(1875066, 1875066));

    pub const fn packed(vec: &Vec2<i32>) -> u64 {
    ...
    pub const fn start_block_x(vec: &Vec2<i32>) -> i32 {
        vec.x << 4
    }

    pub const fn end_block_x(vec: &Vec2<i32>) -> i32 {
        start_block_x(vec) + 15
    }

    pub const fn start_block_z(vec: &Vec2<i32>) -> i32 {
        vec.z << 4
    }

    pub const fn end_block_z(vec: &Vec2<i32>) -> i32 {
        start_block_z(vec) + 15
    }

    pub const fn to_chunk_pos(vec: &Vec2<i32>) -> Vec2<i32> {
        Vec2::new(vec.x >> 4, vec.z >> 4)
    }
}
...
mod test {
    ...
    fn test_chunk_packing() {
        let pos = Vec2::new(305135135, -1351513511);
